...
class MarketAnalyzer:
    ...
    def _determine_scenario(
        ...
        giving more weight to strong pattern signals and divergences.
        """
        # Use passed parameters instead of recalculating
        # (Keep these calculations for debugging but use the parameters)
        calc_volatility = self._calculate_volatility(df)
        calc_trend_strength = self._calculate_trend_strength(df)
        calc_volume = self._analyze_volume_profile(df.tail(30))
        
        print(f"DEBUG: Parameter volatility={volatility}, calculated={calc_volatility}")
        print(f"DEBUG: Parameter trend_strength={trend_strength}, calculated={calc_trend_strength}")
        print(f"DEBUG: Parameter volume_profile={volume_profile}, calculated={calc_volume}")
        
        # Detect market structure
        structure = self._determine_market_structure(df)
        
        # Analyze pattern influence
        pattern_influence = self._analyze_pattern_influence(patterns)
        
        # Add debug output for pattern influence
        print(f"DEBUG: Pattern influence: {pattern_influence}")
        
        # Check for divergence
        has_divergence = self._check_for_divergence(df)
        print(f"DEBUG: Divergence: {has_divergence}")
        
        # Lower thresholds slightly to increase matching probability
        
        # 1. Reversal Zone
        if (has_divergence or 
            (pattern_influence.get("reversal_dominant", 0) > 0.5 and 
            self._is_near_key_level(df, pattern_influence.get("reversal_direction")))):
            return MarketScenario.REVERSAL_ZONE
        
        # 2. Breakout Build-up
        if structure == "contraction" and volatility < 0.5 and \
        (pattern_influence.get("bilateral_dominant", 0) > 0.4 or 
            pattern_influence.get("continuation_dominant", 0) > 0.3):
            return MarketScenario.BREAKOUT_BUILDUP
        
        # 3. Trending
        if abs(trend_strength) > 0.4 and (
        pattern_influence.get("continuation_dominant", 0) > 0.4 or 
        structure in ["strong_uptrend", "strong_downtrend"]):
            return MarketScenario.TRENDING_UP if trend_strength > 0 else MarketScenario.TRENDING_DOWN
        
        # 4. Consolidation
        if structure == "consolidation" and volatility < 0.6:
            return MarketScenario.CONSOLIDATION
        
        # 5. Accumulation/Distribution
        if structure in ["consolidation", "range_bound"]:
            swings = self._identify_swing_points(df.tail(20))
            if swings.get('higher_lows', False) and trend_strength < 0.3:
                return MarketScenario.ACCUMULATION
            elif swings.get('lower_highs', False) and trend_strength > -0.3:
                return MarketScenario.DISTRIBUTION
        
        # 6-8. Volatility scenarios
        if volatility > 0.7:
            return MarketScenario.HIGH_VOLATILITY
        elif volatility < 0.2:
            return MarketScenario.LOW_VOLATILITY
        elif abs(trend_strength) < 0.4 and volatility > 0.4:
            return MarketScenario.CHOPPY
        
        # Clear fallback logic based on trend strength
        ...
        # Final fallback
        return MarketScenario.UNDEFINED  # Better default than UNDEFINED
